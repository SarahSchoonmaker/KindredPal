...
router.get("/debug/discover", auth, async (req, res) => {
  try {
    const currentUser = await User.findById(req.userId)
      .select(
        "name email city state locationPreference filterPoliticalBeliefs likes passed matches blockedUsers",
      )
      .lean();
...
// ===== DELETE ACCOUNT =====
// FIX: replaced user.softDelete() which may not exist on User model
// with a direct findByIdAndUpdate — guaranteed to work regardless of model methods
router.delete("/account", auth, async (req, res) => {
  try {
    ...
    // Soft delete the user — scramble email so it can be reused, clear sensitive data
    ...
    // Remove from other users' matches arrays
