            ...
            {!form.isNationwide && (
              <>
                <input
                  ...
                  placeholder="Street address or venue"
            ...
            {eligible.map((conn) => (
              <div key={conn.user._id} className="invite-row">
                ...
                <div className="invite-info">
                  ...
                  <span className="invite-location">
...
function CreateEventModal({
  ...
  const handleSave = async (e) => {
    ...
    if (!form.isVirtual && !form.city.trim())
      return setError("City is required for in-person events");
    setSaving(true);
    try {
      const res = await eventsAPI.createEvent(groupId, {
        ...form,
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        className="modal-card modal-card-tall"
        ...
        <form onSubmit={handleSave}>
          <div className="modal-field">
            ...
            <input
              ...
              maxLength={100}
              placeholder="e.g. Monthly Brunch, Bible Study"
            />
          </div>
          <div className="modal-field">
            <label>Description</label>
            ...
            <>
              <div className="modal-field">
                ...
                <input
                  ...
                  value={form.address}
                  onChange={(e) => set("address", e.target.value)}
                  placeholder="e.g. 123 Main St, Hangers Cafe"
                />
              </div>
              <div className="location-row">
                <input
        ...
        <div className="event-attendees-preview">
          ...
          <span className="attendee-names-hint">
            {event.goingPreview
              ...
              .join(", ")}
            {event.goingCount > 2
              ? ` & ${event.goingCount - 2} others going`
              : " going"}
          </span>
        </div>
      )}
          ...
          {event.maxAttendees && (
            <span className="event-capacity">
              {event.goingCount >= event.maxAttendees
                ? "🔴 Full"
                : `${event.maxAttendees - event.goingCount} spots left`}
            </span>
          )}
        </div>
        <div className="event-rsvp-btns">
          {event.isGoing ? (
            <>
              ...
              <button
                className="btn-rsvp btn-rsvp-cancel"
                disabled={loading}
            ...
            <>
              ...
              <button
                className="btn-rsvp btn-rsvp-going"
                disabled={loading}
...
export default function GroupDetailPage() {
  ...
  const handleRsvpUpdate = (eventId, data) => {
    setEvents((prev) =>
      prev.map((e) => (e._id === eventId ? { ...e, ...data } : e)),
    );
  };
  ...
  const handleDeleteEvent = async (eventId) => {
    ...
    try {
      await eventsAPI.deleteEvent(groupId, eventId);
  ...
  return (
    <div className="group-detail-page">
      ...
      <div className="group-hero">
        ...
        >
          {group.coverPhoto ? (
            ...
            <span className="group-category-icon">
        ...
        <div className="group-hero-action">
          {group.isAdmin ? (
            ...
            <div className="group-rsvp-btns">
              <p className="group-rsvp-label">
                You've been invited to join this group:
              </p>
              <div className="group-rsvp-row">
                <button
                  className="btn-rsvp-accept"
                  onClick={() => handleRsvpResponse("accept")}
                >
                  ✓ Accept
                </button>
                <button
                ...
                <button
                  className="btn-rsvp-cant"
                  onClick={() => handleRsvpResponse("decline")}
                >
                  ✕ Can't Make It
                </button>
              </div>
              ...
              <div key={req._id} className="member-row">
                ...
                >
                  <button
                    ...
                    onClick={() =>
                      groupsAPI
                        .approveRequest(groupId, req._id)
                        .then(fetchGroup)
                    }
                  >
                  ...
                  <button
                    className="btn-remove-member"
                    onClick={() =>
                      groupsAPI.rejectRequest(groupId, req._id).then(fetchGroup)
                    }
                    title="Reject"
                  >
                    <UserX size={14} />
      ...
      <div className="group-detail-tabs">
        ...
        <button
          className={`gd-tab ${activeTab === "events" ? "active" : ""}`}
          ...
          {!group.isNationwide && (
            <div className="group-section">
              ...
              >
                ...
                <span className="location-maps-hint">
                  Tap to open in Maps ↗
                </span>
              </div>
            </div>
        ...
        <div className="group-section">
          ...
          {eventsLoading ? (
            ...
            <div className="events-empty">
              ...
              {group.isAdmin && (
                <button
                  className="btn-create-event-empty"
                  onClick={() => setShowCreateEvent(true)}
                >
